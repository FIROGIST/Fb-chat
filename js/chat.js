...
class FirebaseChat {
    ...
    initNotificationSound() {
        try {
            this.notificationSound = new Audio('https://www.soundjay.com/buttons/sounds/button-16.mp3');
    ...
    playNotificationSound() {
        try {
            if (this.notificationSound) {
                ...
                this.notificationSound.play().catch(e => console.log('مش قادر يشغل الصوت:', e));
            }
        } catch (error) {
    ...
    // تأثير انفجار وقلوب
    createExplosionEffect(messageElement) {
        try {
            ...
            for (let i = 0; i < 30; i++) {
                ...
                setTimeout(() => {
                    particle.style.transform = `translate(${dx}px, ${dy}px) scale(0.2) rotate(${Math.random() * 720}deg)`;
    ...
    isSpoiled(username) {
        return username && username.toLowerCase() === this.spoiledUsername.toLowerCase();
    }
    ...
    getDisplayName(user) {
        ...
        if (this.isSpoiled(user.username)) {
            return `<span class="spoiled-name">${this.escapeHtml(user.name)} <span class="green-check">✓</span></span>`;
        }
    ...
    getDisplayUsername(user) {
        ...
        if (this.isSpoiled(user.username)) {
            return `@${this.escapeHtml(user.username)} <span class="spoiled-badge">مغرورة</span>`;
        }
        ...
        try {
            ...
            if (storyDoc.exists) {
                ...
                if (!views.includes(this.currentUser.username)) {
                    ...
                    await storyRef.update({ views: views });
                }
            }
        } catch (error) {
    ...
    displayMessage(messageData) {
        ...
        // تأثير الانفجار لكلمة "بسبوسه"
        if (messageData.message && messageData.message.includes('بسبوسه')) {
            setTimeout(() => {
                this.createExplosionEffect(messageElement);
            }, 200);
        }
    }
